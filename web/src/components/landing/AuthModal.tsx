  ...
  return (
    <div className="fixed inset-0 z-[140] flex items-center justify-center bg-[rgba(0,0,0,0.76)] px-4 backdrop-blur-[12px]" onClick={onClose}>
      <div className="w-full max-w-[420px] rounded-[16px] border border-[rgba(170,255,0,0.24)] bg-[linear-gradient(165deg,rgba(20,35,18,0.2),rgba(15,22,18,0.3))] p-8 shadow-[0_24px_64px_rgba(0,0,0,0.5),0_0_0_1px_rgba(170,255,0,0.06),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-[22px]" onClick={e => e.stopPropagation()}>
        ...
        {step === 0 && (
          <>
            <div className="mb-2 text-[10px] uppercase tracking-[0.07em] text-[var(--oc-lime)]">Crear perfil</div>
            <h3 className="text-white text-[20px] font-medium tracking-[-0.02em] mb-1">¿Quién sos?</h3>
            <p className="mb-5 text-[12px] text-[var(--oc-text-muted)]">Elegí tu rol para continuar.</p>
            <div className="flex flex-col gap-2">
              {roles.map(r => (
                ...
                >
                  ...
                  <span className="text-[13px] text-[var(--oc-text-primary)]">{r.label}</span>
        ...
        {step === 1 && (
          <>
            <div className="mb-2 text-[10px] uppercase tracking-[0.07em] text-[var(--oc-lime)]">Crear perfil · {role}</div>
            <h3 className="text-white text-[20px] font-medium tracking-[-0.02em] mb-4">Comenzá tu registro</h3>
            <div className="space-y-2 mb-4">
              <input className="h-[40px] w-full rounded-[8px] border border-[var(--oc-border-strong)] bg-[var(--oc-surface-2)] px-3 text-[12px] text-white outline-none placeholder:text-[var(--oc-text-faint)] focus-visible:border-[var(--oc-border-green)]" placeholder="Nombre completo" />
              <input className="h-[40px] w-full rounded-[8px] border border-[var(--oc-border-strong)] bg-[var(--oc-surface-2)] px-3 text-[12px] text-white outline-none placeholder:text-[var(--oc-text-faint)] focus-visible:border-[var(--oc-border-green)]" placeholder="Correo" />
              <input className="h-[40px] w-full rounded-[8px] border border-[var(--oc-border-strong)] bg-[var(--oc-surface-2)] px-3 text-[12px] text-white outline-none placeholder:text-[var(--oc-text-faint)] focus-visible:border-[var(--oc-border-green)]" placeholder="País" />
            </div>
