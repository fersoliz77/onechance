...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    if (prefersReduced) {
      const reducedFrame = window.requestAnimationFrame(() => setHeroMetrics(targets))
  ...
  return (
    <main id="main-content" className="bg-[var(--oc-bg-base)] text-white">
      <section ref={heroRef} aria-label="Presentación principal" className="relative min-h-[100svh] overflow-hidden pb-20 pt-[calc(var(--oc-nav-height)+56px)] max-sm:pb-36 max-sm:pt-[calc(var(--oc-nav-height)+40px)]">
        <div className="absolute inset-0" aria-hidden="true">
          <div className="absolute inset-0" style={parallaxBgStyle}>
            <Image
              src="/images/hero-stadium.png"
              alt="Estadio de fútbol visto desde las tribunas"
              fill
              priority
              fetchPriority="high"
              sizes="100vw"
        ...
        <div className="oc-shell relative z-10 grid min-h-[calc(100svh-120px)] items-center gap-12 py-12 max-sm:gap-7 max-sm:py-10 xl:grid-cols-[1.4fr_1fr] xl:gap-12 xl:py-16 2xl:gap-16 2xl:py-20">
          <div className="max-sm:-translate-y-[58px] xl:self-center">
            <div className="mb-7 inline-flex items-center gap-2.5 rounded-full border border-[rgba(255,255,255,0.12)] bg-[rgba(255,255,255,0.04)] px-[14px] py-1.5 text-[13px] font-[500] tracking-[0.02em] text-white/90 backdrop-blur-[4px]">
              <span className="h-1.5 w-1.5 rounded-full bg-[var(--oc-lime)] shadow-[0_0_0_3px_rgba(170,255,0,0.18)] animate-[oc-pulse_2.4s_infinite]" aria-hidden="true" />
            ...
            <p className="mt-4 text-[21px] italic font-[600] tracking-[-0.01em] text-[var(--oc-lime)] xl:mt-3">
              Una oportunidad puede cambiarlo todo.
            </p>
            <p className="mt-5 max-w-[480px] text-[16px] leading-[1.7] text-[var(--oc-fg-muted)] xl:mt-4">
              La vidriera profesional donde jugadores, jugadoras, clubes, técnicos y representantes
              se conectan con oportunidades reales. Mostrá tu talento con un perfil visual,
              ordenado y pensado para scouting.
            </p>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-3.5 pb-2 max-sm:mb-6 xl:mt-6">
              ...
              <Link
                href="/jugadores"
                className="inline-flex h-[50px] min-w-[220px] items-center justify-center rounded-[8px] border border-[var(--oc-border-hi)] bg-transparent px-6 text-[16px] font-[700] leading-none text-white transition-all hover:border-white hover:bg-[rgba(255,255,255,0.05)] max-sm:w-full"
              >
                Explorar talentos
              </Link>
            </div>
            ...
            <div
              ref={metricsRef}
              className="mt-12 flex max-w-[480px] flex-wrap items-center gap-x-6 gap-y-4 rounded-[12px] border border-[rgba(255,255,255,0.08)] bg-[linear-gradient(160deg,rgba(255,255,255,0.03)_0%,transparent_100%)] px-6 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-[12px] sm:justify-between xl:mt-8"
            ...
            >
              {([
                ...
                <div key={label} className="flex flex-col items-start">
                  <div className="text-[21px] font-[800] leading-none tracking-[-0.02em] text-[var(--oc-lime)]" aria-label={`${value} ${label}`}>{value}</div>
                  <div className="mt-1 text-[12px] font-[500] uppercase tracking-[0.04em] text-[var(--oc-fg-dim)]" aria-hidden="true">{label}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Tarjetas 3D hero */}
          <div
            className="relative h-[500px] w-full min-w-[280px] [perspective:1400px] max-sm:h-[430px] xl:mx-0 xl:self-center max-lg:mx-auto max-lg:max-w-[440px]"
          ...
          >
            <article className="absolute inset-[20px_80px_60px_0] overflow-hidden rounded-[12px] border border-[rgba(170,255,0,0.28)] bg-[linear-gradient(165deg,rgba(20,35,18,0.22),rgba(15,22,18,0.3))] p-6 shadow-[0_24px_64px_rgba(0,0,0,0.5),0_0_0_1px_rgba(170,255,0,0.06),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-[22px] [transform:perspective(1400px)_rotateY(-6deg)_rotateX(3deg)] [transform-style:preserve-3d] [animation:oc-tilt-float_6s_ease-in-out_infinite] max-lg:inset-[30px_60px_80px_0] max-sm:inset-[30px_40px_100px_0] max-sm:p-5">
              ...
              <div className="relative z-10">
                ...
                <div className="mt-[22px] flex flex-col gap-2.5">
                  {[['📅', '19 años'], ['🌐', 'Argentina'], ['🎥', '8 videos']].map(([icon, text]) => (
                    <div key={text} className="flex items-center gap-3 rounded-[8px] border border-[rgba(170,255,0,0.14)] bg-[rgba(170,255,0,0.06)] px-3 py-2.5 text-[14px] text-[rgba(255,255,255,0.92)] backdrop-blur-[4px]">
                      <span className="text-[17px] leading-none">{icon}</span>{text}
                    </div>
                  ))}
                </div>
            ...
            <div className="absolute bottom-[20px] right-[20px] z-30 w-[200px] rounded-[10px] border border-[rgba(255,255,255,0.14)] bg-[rgba(20,22,26,0.3)] p-[14px_16px] shadow-[0_16px_40px_rgba(0,0,0,0.5),inset_0_1px_0_rgba(255,255,255,0.1)] backdrop-blur-[24px] [transform-style:preserve-3d] [animation:oc-tilt-soft-float_7s_ease-in-out_infinite] [animation-delay:-3s] max-lg:bottom-[10px] max-lg:right-0 max-sm:bottom-0 max-sm:right-0 max-sm:w-[170px] max-sm:p-3 overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <svg className="mt-2 h-[36px] w-full" viewBox="0 0 200 36" preserveAspectRatio="none" aria-hidden="true">
                  <g fill="#AAFF00">
                    {[[2,22,12,0.5],[22,18,16,0.55],[42,24,10,0.5],[62,14,20,0.65],[82,20,14,0.55],[102,10,24,0.75],[122,16,18,0.65],[142,6,28,0.85],[162,12,22,0.75],[182,2,32,1]].map(([x,y,h,o],i) => (
                      <rect key={i} x={x} y={y} width="14" height={h} rx="1" opacity={o} />
                    ))}
                  </g>
                </svg>
              </div>
      ...
      <footer className="border-t border-[var(--oc-border)] py-8">
        <div className="oc-shell flex flex-wrap items-center justify-between gap-4 text-[14px] text-[var(--oc-fg-dim)]">
          <div>© 2026 OneChance. Todos los derechos reservados.</div>
          <nav aria-label="Links del pie de página">
            <ul className="flex gap-4 list-none m-0 p-0">
              ...
              <li><a href="mailto:contacto@onechance.app" className="hover:text-white transition-colors">Contacto</a></li>
            </ul>
          </nav>
        </div>
      </footer>
