    ...
    <EntityCardShell onClick={onClick} tone="yellow" className="min-h-0 p-0 overflow-hidden">
      <div className="grid lg:grid-cols-[240px_1fr_260px]">
        ...
        <div className="border-b border-[var(--oc-border-soft)] px-5 py-4 lg:border-b-0 lg:border-r lg:px-6 lg:py-5">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="truncate text-[20px] font-semibold tracking-[-0.03em] text-white">{club.name}</div>
              <div className="mt-1 text-[12px] text-[var(--oc-text-muted)]">{club.country || 'Sin pais'} · {club.city || 'Sin ciudad'}</div>
            </div>
            <span className="rounded-md border border-[rgba(255,180,0,0.35)] bg-[rgba(255,180,0,0.12)] px-2.5 py-1 text-[11px] font-medium text-oc-yellow">
              {club.division || 'Sin categoria'}
            </span>
          </div>
          <div className="mt-4 grid gap-2 text-[12px] text-[var(--oc-text-muted)] lg:grid-cols-2">
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-4 lg:flex-col lg:items-start lg:justify-center lg:px-6">
          <div>
            <p className="text-[11px] uppercase tracking-[0.08em] text-[var(--oc-text-faint)]">Estado del perfil</p>
            <p className="mt-1 text-[13px] text-white">Verificado y publicado</p>
          </div>
          <span className="ml-auto text-[12px] text-[var(--oc-text-faint)] transition-colors group-hover:text-oc-yellow lg:ml-0">Ver perfil →</span>
      </div>
      </div>
    </EntityCardShell>
  )
