    ...
    <EntityCardShell onClick={onClick} tone="purple" className="rounded-[var(--oc-radius-xl)] bg-[rgba(7,20,24,0.78)] shadow-[0_0_0_1px_rgba(0,212,255,0.04),0_18px_50px_rgba(0,0,0,0.35)]">
      <div className="flex items-start gap-3 lg:gap-3.5 mb-3 lg:mb-3.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="text-white text-[15px] lg:text-[16px] font-medium truncate">{agent.fullName}</div>
          <div className="text-[12px] lg:text-[13px] mt-0.5 text-oc-purple">{agent.agencyName || 'Independiente'}</div>
        </div>
        {agent.players > 0 && (
          <div className="rounded-[7px] px-[8px] py-[5px] text-center shrink-0 bg-[rgba(180,100,255,0.12)] border border-[rgba(180,100,255,0.24)]">
            <div className="text-[17px] lg:text-[18px] font-medium leading-none text-oc-purple">{agent.players}</div>
            <div className="text-[var(--oc-text-faint)] text-[8px] uppercase mt-0.5 tracking-[0.04em]">JUGAD.</div>
          </div>
        )}
      </div>
      ...
      <div className="flex justify-between items-center">
        <span className="text-[var(--oc-text-faint)] text-[12px]">{agent.countries > 0 ? `${agent.countries} países` : 'Mercado local'}</span>
        <span className="text-[13px] text-[var(--oc-text-faint)] transition-colors group-hover:text-oc-purple">Ver perfil →</span>
      </div>
    </EntityCardShell>
  )
}
